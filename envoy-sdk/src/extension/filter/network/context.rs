...
impl<'a, F> proxy_wasm::traits::StreamContext for FilterContext<'a, F> where F: super::Filter {
    ...
    fn on_downstream_data(&mut self, data_size: usize, end_of_stream: bool) -> Action {
        self.filter.on_downstream_data(data_size, end_of_stream, self.logger_ops.as_downstream_data_ops()).unwrap()
    }
    ...
    fn on_upstream_data(&mut self, data_size: usize, end_of_stream: bool) -> Action {
        self.filter.on_upstream_data(data_size, end_of_stream, self.logger_ops.as_upstream_data_ops()).unwrap()
    }
...
impl<'a, F> proxy_wasm::traits::Context for FilterContext<'a, F> where F: super::Filter {
    ...
    fn on_http_call_response(&mut self, token_id: u32, num_headers: usize, body_size: usize, num_trailers: usize) {
        self.filter.on_http_call_response(token_id, num_headers, body_size, num_trailers, self.logger_ops, self.http_client_ops).unwrap()
    }
}

impl<'a, F> FilterContext<'a, F> where F: super::Filter {
    pub fn new(filter: F, logger_ops: &'a dyn super::Ops, http_client_ops: &'a dyn clients::http::ResponseOps) -> FilterContext<'a, F> {
        FilterContext {
            filter: filter,
