...
fn test_headers() -> Result<()> {
    ...
    assert_eq!(
        fake_info.request_header("content-type")?,
        Some("application/json".into())
    );
    assert_eq!(fake_info.request_header("x-custom-header")?, None);

    assert_eq!(
        fake_info.response_headers()?,
        HeaderMap::builder()
            .header(":status", "201")
            .header("content-type", "application/json")
            .header("content-length", "2002")
    ...
    assert_eq!(
        fake_info.response_header("content-length")?,
        Some("2002".into())
    );
    assert_eq!(fake_info.response_header("x-custom-header")?, None);

    assert_eq!(
        fake_info.response_trailers()?,
        HeaderMap::builder()
    ...
    assert_eq!(
        fake_info.response_trailer("grpc-message")?,
        Some("UNKNOWN".into())
    );
    assert_eq!(fake_info.response_trailer("x-custom-trailer")?, None);
...
fn test_connection() -> Result<()> {
    ...
    assert_eq!(fake_info.stream_info().connection().id()?, Some(123));
    assert_eq!(fake_info.stream_info().connection().is_mtls()?, Some(true));
    assert_eq!(
        fake_info
            .stream_info()
            .connection()
    ...
    assert_eq!(
        fake_info.stream_info().connection().tls().version()?,
        Some("TLSv1.2".into())
    );
    assert_eq!(
        fake_info
...
fn test_http_request() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().request().id()?,
        Some("a-b-c-d".to_owned())
    );
    assert_eq!(
        fake_info.stream_info().request().time()?,
        Some(SystemTime::UNIX_EPOCH + Duration::from_secs(12))
    );
    assert_eq!(
        fake_info.stream_info().request().duration()?,
        Some(Duration::from_millis(123))
    );
    assert_eq!(fake_info.stream_info().request().size()?, Some(1024));
    assert_eq!(fake_info.stream_info().request().total_size()?, Some(2048));
    assert_eq!(
        fake_info.stream_info().request().method()?,
        Some("GET".into())
    );
    assert_eq!(
        fake_info.stream_info().request().scheme()?,
        Some("https".into())
    );
    assert_eq!(
        fake_info.stream_info().request().host()?,
        Some("www.example.com".into())
    );
    assert_eq!(
        fake_info.stream_info().request().path()?,
        Some("/search?q=example".into())
    );
    assert_eq!(
        fake_info.stream_info().request().url_path()?,
        Some("/search".into())
    );
    assert_eq!(
        fake_info.stream_info().request().protocol()?,
        Some("HTTP/1.1".into())
    );
    assert_eq!(
        fake_info.stream_info().request().header("content-length")?,
        Some("1001".into())
    );
    assert_eq!(
        fake_info.stream_info().request().user_agent()?,
        Some("curl".into())
    );
    assert_eq!(
        fake_info.stream_info().request().referer()?,
...
fn test_http_response() -> Result<()> {
    ...
    assert_eq!(fake_info.stream_info().response().status_code()?, Some(201));
    assert_eq!(fake_info.stream_info().response().size()?, Some(1024));
    assert_eq!(fake_info.stream_info().response().total_size()?, Some(2048));
    assert_eq!(fake_info.stream_info().response().grpc_status()?, Some(1));
    assert_eq!(
        fake_info.stream_info().response().flags()?,
        Some(ResponseFlags::FAILED_LOCAL_HEALTH_CHECK | ResponseFlags::DELAY_INJECTED)
    );
    assert_eq!(
        fake_info.stream_info().response().header("content-type")?,
        Some("application/json".into())
    );
    assert_eq!(
        fake_info.stream_info().response().trailer("grpc-message")?,
        Some("UNKNOWN".into())
    );
...
fn test_upstream() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().upstream().address()?,
        Some("192.168.0.1".to_owned())
    );
    assert_eq!(fake_info.stream_info().upstream().port()?, Some(5432));
    assert_eq!(
        fake_info.stream_info().upstream().local_address()?,
    ...
    assert_eq!(
        fake_info.stream_info().upstream().tls().version()?,
        Some("TLSv1.1".into())
    );
    assert_eq!(
        fake_info
...
fn test_source() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().source().address()?,
        Some("10.0.0.1".to_owned())
    );
    assert_eq!(fake_info.stream_info().source().port()?, Some(50505));
...
fn test_destination() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().destination().address()?,
        Some("172.0.0.1".to_owned())
    );
    assert_eq!(fake_info.stream_info().destination().port()?, Some(15000));
...
fn test_listener() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().listener().traffic_direction()?,
...
fn test_route() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().route().name()?,
        Some("my_route".to_owned())
    );
...
fn test_cluster() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().cluster().name()?,
...
fn test_plugin() -> Result<()> {
    ...
    assert_eq!(
        fake_info.stream_info().plugin().name()?,
        Some("my_plugin".to_owned())
    );
    assert_eq!(
        fake_info.stream_info().plugin().root_id()?,
        Some("my_root_id".to_owned())
    );
    assert_eq!(
        fake_info.stream_info().plugin().vm_id()?,
        Some("my_vm_id".to_owned())
    );
