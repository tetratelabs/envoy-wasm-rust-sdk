...
impl<'a> FakeHttpStream<'a> {
        ...
        B: AsRef<[u8]>,
    {
        ...
        match status {
            Ok(http::FilterDataStatus::Continue) => {
                ...
                match iteration_state {
                    ...
                    IterationState::StopSingleIteration => {
                        let data: Vec<u8> =
                            self.state.borrow_mut().request_data.drain(..).collect();
                        self.state.borrow_mut().request_buffered_data.extend(data);
                    }
                    _ => unimplemented!(),
                };
            ...
            Ok(http::FilterDataStatus::StopIterationAndBuffer) => {
                let data: Vec<u8> = self.state.borrow_mut().request_data.drain(..).collect();
                self.state.borrow_mut().request_buffered_data.extend(data);
        ...
        if let Ok(status) = status {
            self.state.borrow_mut().request_flow.body = Some(status);
        }
        status
    }
        ...
        H: Into<HeaderMap>,
    {
            ...
            Ok(http::FilterTrailersStatus::Continue) => {
                ...
                match iteration_state {
                    IterationState::Continue => {
                        ...
                        let end_of_stream = self.state.borrow().request_ended;
                        self.state
                            .borrow_mut()
                            .upstream
        ...
        if let Ok(status) = status {
            self.state.borrow_mut().request_flow.trailers = Some(status);
        }
        status
    }
        ...
        H: Into<HeaderMap>,
    {
        ...
        match status {
            Ok(http::FilterHeadersStatus::Continue) => {
                self.state.borrow_mut().response_flow.iteration_state = IterationState::Continue;
            ...
            Ok(http::FilterHeadersStatus::StopIteration) => {
                self.state.borrow_mut().response_flow.iteration_state =
                    IterationState::StopSingleIteration;
            }
            Ok(status) => panic!(
                "oops, it seems that test framework is no longer up-to-date: unknown status {:?}",
        ...
        B: AsRef<[u8]>,
    {
        ...
        self.state.borrow_mut().response_ended |= end_of_stream;
        self.state.borrow_mut().response_data = data.as_ref().to_vec();

        let buf_len = self.response_data().len();
        let status = self
            .filter
            .borrow_mut()
        ...
        match status {
            Ok(http::FilterDataStatus::Continue) => {
                let iteration_state = self.state.borrow().response_flow.iteration_state;
                match iteration_state {
                    IterationState::Continue => {
                        let data = self.state.borrow_mut().response_data.drain(..).collect();
                        self.state
                            .borrow_mut()
                            .downstream
                            .receive_data(data, end_of_stream);
                    ...
                    IterationState::StopSingleIteration => {
                        let data: Vec<u8> =
                            self.state.borrow_mut().response_data.drain(..).collect();
                        self.state.borrow_mut().response_buffered_data.extend(data);
                    }
                    _ => unimplemented!(),
                };
            ...
            Ok(http::FilterDataStatus::StopIterationAndBuffer) => {
                let data: Vec<u8> = self.state.borrow_mut().response_data.drain(..).collect();
                self.state.borrow_mut().response_buffered_data.extend(data);
...
impl<'a> http::RequestBodyOps for FakeHttpStream<'a> {
    fn request_data(&self, offset: usize, max_size: usize) -> host::Result<ByteString> {
        simulate::get_buffer_bytes(&self.request_data(), offset, max_size)
    }

    fn mutate_request_data(&self, change: Transform) -> host::Result<()> {
        change.execute(|start: usize, length: usize, data: &[u8]| {
            simulate::set_buffer_bytes(&mut self.request_data_mut(), start, length, data)
        })
    }
}
...
impl<'a> http::ResponseBodyOps for FakeHttpStream<'a> {
    fn response_data(&self, offset: usize, max_size: usize) -> host::Result<ByteString> {
        simulate::get_buffer_bytes(&self.response_data(), offset, max_size)
    }

    fn mutate_response_data(&self, change: Transform) -> host::Result<()> {
        change.execute(|start: usize, length: usize, data: &[u8]| {
            simulate::set_buffer_bytes(&mut self.response_data_mut(), start, length, data)
        })
    }
}
