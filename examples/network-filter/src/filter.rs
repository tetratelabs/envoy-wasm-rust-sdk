...
impl<'a> SampleNetworkFilter<'a> {
    pub fn new(config: Rc<SampleNetworkFilterConfig>, instance_id: u32, time_service: &'a dyn time::Service, http_client: &'a dyn clients::http::Client) -> SampleNetworkFilter<'a> {
        SampleNetworkFilter {
            config: config,
...
impl<'a> network::Filter for SampleNetworkFilter<'a> {
    fn on_new_connection(&mut self) -> Result<network::FilterStatus> {
        ...
        info!("#{} new TCP connection starts at {} with config: {}", self.instance_id, datetime.format("%+"), self.config.value);
